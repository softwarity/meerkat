...
| THEME-05 | **Gestion du color-scheme utilisateur** (clair/sombre/système) : préférence portée par le **profil utilisateur** et mémorisée côté navigateur (utilisable avant login) ; les **pages servies par la gateway** (login, sélections, profil…) la respectent — la console d'admin peut rester sombre. Pour chaque **route/service UI**, l'admin décrit **comment l'application hébergée reçoit le scheme** (nom d'attribut et valeurs — ex. `data-theme="dark"`, une classe CSS, ou rien si l'app suit `prefers-color-scheme`) et **le code injecté par la gateway l'applique pour elle** : les applications n'implémentent aucun toggle. | ✘ | M |
...
| I18N-04 | **Dispatch de la locale aux applications hébergées** : la langue de l'utilisateur (choisie dans son profil, ou détectée navigateur à défaut — CONSOLE-09) est **transmise à chaque application UI selon son implémentation à elle**, configurable par route/service : en-tête (`Accept-Language` ou header custom), segment de chemin (template `{language}`), paramètre de query, cookie. Le sélecteur de langue injecté (UIF-05) met à jour le profil, et toutes les applications suivent. | ◐ | M |
