...
| TENANT-04 | **Plages d'accès métier (business access)** : restriction de connexion par plages horaires, jours de semaine, dates de début/fin, fuseau ; définies globalement, surchargées par organisation puis par membre. | ◐ | S |
| TENANT-05 | **TTL de session hiérarchique** : global (défaut 30 min) → par organisation → par membre. | ✔ | S |
